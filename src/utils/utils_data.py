...
from dataset_sim_hospital              import zospital_object
from dataset_assemble_sim              import assemble_object  # this is static env


def gather_all_data(data_dir:str, past:int, maxT:int, minT:int=1, period:int=1, save_dir:str|None=None):
    if save_dir is None:
        save_dir = data_dir
...
def save_HPD_data(index_list:list, save_path:str, sim_time_per_scene:int, test=False, version='hospital'):
    # Hospital Dataset
    if version == 'hospital':
        size_iches = (4.01, 4.01)
        stagger = 1 + random.randint(1, 5)
        vmax = 11 + random.randint(1, 12)
        GraphClass = hospital_object.Graph
        ObjClass = hospital_object.MovingObject
    elif version == 'zospital':
        size_iches = (3.21, 3.21)
        stagger = 1 + random.randint(1, 5)
        vmax = 8 + random.randint(1, 14) # 1m = 20px, reasonable speed is 10~22px/s 
        GraphClass = zospital_object.Graph
        ObjClass = zospital_object.MovingObject
    else:
        raise NotImplementedError(f'Version {version} not implemented.')

    root_dir = Path(__file__).resolve().parents[2]
    map_path = os.path.join(root_dir, 'src', f'dataset_sim_{version}/{version}_sim_original', 'label.png')

    ts = 0.2 # sampling time
    overall_sim_time = sim_time_per_scene * len(index_list)
    cnt = 0
    graph = GraphClass(map_path)
    for start_idx in index_list:
        t = 0
        t_list = []   # time or time step
        id_list = []
        ...
        for _ in range(sim_time_per_scene):
            ...
            obj = ObjClass(ref_path[0], stagger=stagger)
            obj.run(ref_path, ts, vmax)
        ...
        ax.set_aspect('equal')
        ax.axis('off')
        fig.set_size_inches(*size_iches) # XXX depends on your dpi!
        fig.tight_layout(pad=0)
        fig.savefig(os.path.join(save_path, f'{start_idx}', 'background.png'))
        plt.close()
...
def gather_all_data_position(data_dir:str, past:int, maxT:int, minT:int=1, period:int=1, save_dir:str|None=None):
...
@deprecated # This was used for WTA single position prediction, should not be used in the future
def gather_all_data_trajectory(data_dir:str, past:int, maxT:int, minT:int=1, period:int=1, save_dir:str|None=None):
    if save_dir is None:
        save_dir = data_dir
